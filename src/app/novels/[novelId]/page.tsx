...
export default function NovelPage({ params }: { params: { novelId: string } }) {
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-4">{getText(novel.title, language)}</h1>
        <p className="text-lg text-gray-600 dark:text-gray-400 mb-2">by {novel.author}</p>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
          <div>
            <p className="text-sm text-gray-600 dark:text-gray-400">{labels.genre}</p>
            <p className="font-medium">{getText(novel.genre, language)}</p>
          </div>
          <div>
            <p className="text-sm text-gray-600 dark:text-gray-400">{labels.status}</p>
            <p className="font-medium">{getText(novel.status, language)}</p>
          </div>
          <div>
            <p className="text-sm text-gray-600 dark:text-gray-400">{labels.chapters}</p>
            <p className="font-medium">{novel.chapters.length}</p>
          </div>
          <div>
            <p className="text-sm text-gray-600 dark:text-gray-400">{labels.lastUpdated}</p>
      ...
      <div>
        <h2 className="text-2xl font-semibold mb-4">{labels.tableOfContents}</h2>
            ...
            >
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">{chapter.publishDate}</p>
                </div>
                <svg
