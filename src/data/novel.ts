...
Heather turned off the lamp. The room grayed. Rain wrote soft cursive against the glass. The last train rattled somewhere beneath the street, carrying strangers home like commas in a long sentence that would finish itself eventually.`,
        ko: `마감 시간, 카페는 막 하품을 한 것처럼 보였다. 의자들은 테이블 위에 거꾸로 올려져 있었고, 바닥에는 소금기 묻은 발자국이 녹아 어두운 회색 얼룩으로 번져 있었다. 헤더는 데드볼트를 탁 잠그고, 유리에 비친 흔들리는 자기 모습을 바라보았다.

가로등에 비친 유령 같은 얼굴 위로 택시의 노란빛 얼룩이 스쳐 지나갔다. 마치 색이란 그저 지나가기만 하는 것처럼.

밖에는 결심하지 못한 비가 바람을 타고 비스듬히 내렸다. 부드럽고, 끈질기고, 믿음을 다 써버린 눈의 연장선처럼. 모퉁이 맨홀에서는 김이 올라왔다. 도시의 내쉬는 숨. 그녀는 목도리를 턱까지 끌어올리고 회색 속으로 걸음을 옮겼다.

열쇠는 아직 손바닥의 온기를 품고 있었다. 주머니 속 공책은 더 따뜻했다. 일곱 시간을 서 있었지만 어디에도 도착한 느낌이 없었다. 문종은 이백 번쯤 울렸을 것이다. 손은 주문마다 고유한 근육 기억을 갖고 있었다: 포타필터를 두드리고, 피처를 돌리고, 숨을 들이쉬고. 삐딱한 하트가 사랑에서 빠져나오려는 것처럼 보이는 카푸치노를 세 잔 만들었다. 도서관 먼지 냄새가 나는 단골은 또다시 캐모마일을 주문해놓고, 이번에도 손대지 않은 채 기다리기만 했다.

공책에는 작게 기울어진 글씨로 이렇게 적혀 있었다. *"차가 식을 때까지 캐모마일 차를 마시지 않는 남자가 있다."*

밖에서는 타이어가 긴 반사선을 그리며 거리를 칠했다. 횡단보도 페인트 위에서 부츠가 한 번 끽 소리를 냈다가, 다시 아스팔트를 찾았다. 건물 타이머와 함께 카페의 네온사인이 꺼지자, 골목은 한 박자 낮아진 듯 어두워졌다. 마치 노래가 브리지로 넘어가듯.

헤더는 도시가 이렇게 반쯤 잠들어 솔직해질 때가 가장 좋았다.

계단을 내려가자, 지하철은 숨을 멈추고 있었다. 타일 벽은 자신들의 나이를 사과 없이 드러냈다. 작은 강처럼 뻗어가는 잔금들, 세상을 약속하는 공연 포스터들. 회전문이 앓는 소리를 냈다. OMNY 패드가 짧게 '삑' 하고 울렸다. 어딘가에서 버스커의 색소폰이 선율을 기억하려 애쓰다가 하다 마는 듯 포기하는 소리가 들렸다.

승강장에는 사람들이 각자의 피로한 모양으로 몸을 접어 두었다. 십대 둘은 무릎과 웃음을 한 칸의 벤치에 욱여넣듯 바짝 붙어 앉아 있었고, 한 건설 노동자는 장갑 낀 손으로 눈을 비비다가 뺨에 먼지 초승달을 남겼다.

헤더는 노란 선 가장자리에 서서 아래를 내려다보지 않았다. 대신 공책을 꺼냈다. 그녀가 쓰는 연필은 깎인 자국보다 씹힌 자국이 더 많았다. 흑연의 맛이 좋았다. 그럴 때만 뭔가 느끼는 것 같아서.

먼저 바람, 그다음 냄새, 그리고 브레이크 소리—최고급 노이즈 캔슬링 헤드폰도 뚫고 들어오려는 고집. 브레이크는 긴 금속성 모음으로 자신을 드러냈다. 역 전체를 한 음절로 붙잡는 소리. 문은 익숙하고도 참을성 있는 짜증을 내며 열렸다. 그녀는 열차에 올라 기둥 옆에 섰다. 형광등은 모두를 같은 종류의 물고기처럼 보이게 만들었다.

그녀는 자신을 붙잡지도 놓아주지도 않는 카페를 떠올렸다. 시트러스 세제와 커피 오일 냄새, 계산대 근처에 딱지처럼 벗겨진 페인트 자국이 다시 칠해지지 않은 채 남아 있는 곳. 늘 "서프라이즈요"라고 했다가 막상 놀라면 찡그리는 단골. 너무 빨리 연해져 버린 에스프레소 샷. 네 시간쯤 지나면 자동으로 변하는 자신의 목소리—다음 분이요, 영수증은 봉투에, 좋은 밤 보내세요—주머니에 넣어두고 더는 만지지 않는 매끈한 조약돌 같은 말들.

그녀는 다시 공책의 부드러운 모서리를 만졌다. 도시를 적어두는 게, 그 안에서 무언가를 느끼는 것보다 쉬웠다.

맞은편에서, 모직 코트를 입은 남자가 문자 중에 그대로 잠들었다. 전화기는 그의 허벅지 위에 균형을 잡고 있었다. 열차가 흔들려도 죄 없는 낮잠은 흔들리지 않았다. 그 옆에는 스케치북을 펼친 사람이 있었고, 연필은 사각사각, 손목의 작은 튕김과 함께 그을음 같은 음영을 얹고 있었다. 헤더는 의미를 만들 만큼 오래 보지 않았다. 그녀는 눈을 아껴 썼다. 대신 이렇게 썼다.

*손은 우리를 들킨다—바리스타의 손톱 아래엔 커피의 초승달, 학생들의 손등엔 잉크 자국, 겨울 손은 마디가 벌겋다. 예술가의 손목은 물을 흐리듯 움직인다.*

열차가 툭 하고 덜컹였다. 맞은편 광고는 "올바른 매트리스를 사면 3주 안에 숙면을 약속했다." 그녀는 자기 침대를 떠올렸다. 시트가 모서리를 끌어당겨 다른 것이 되려는—돛, 혹은 조용한 깃발이 되려는—모양. 그녀의 아파트에는 벽돌담과 비둘기 기분에 따라 색이 바뀌는 하늘 조각이 보이는 그럭저럭 괜찮은 창이 하나 있었다. 라디에이터가 똑딱거리면 오래된 시계가 아직 작동하는 소리 같아서 가장 좋았다.

유니언 스퀘어에서 객차는 성긴 빗살처럼 비어 갔다. 화분을 안은 여자가 문이 닫힐 때마다 떨리는 잎을 더 꼭 끌어안았다. 헤더는 밤이 되면 뿌리도 깨어나 귀를 기울일 것이라고 상상했다.

그녀는 차가운 금속에 머리를 기대었다. 눈을 감아도 카페의 우유가 하얀 안개로 피어오르는 모습이 보였고, 네 시간 전의 헤더가 "무엇으로 시작해드릴까요?"라고 말하는 소리가 들렸다. 상냥한 목소리였다. 하지만 그녀의 것이 아니었다.

14스트리트에서 네이비색 모자를 쓴 남자가 옆자리에 사람 있냐고 물었다. 그녀는 가방을 옮겼고, 지퍼가 손목을 스쳤다. "감사해요." 그녀는 고개를 끄덕였다. 대화 끝. 도시는 효율적인 교환을 사랑했고—그래야 밤이 흐트러지지 않았다.

열차는 학생, 간호사, 목도리에 공손히 얼굴을 묻고 우는 사람, 낮은 목소리와 값싼 약속으로 미래를 설계하는 연인을 태우고, 한동안 젖어 있기로 결정한 도시를 가로질렀다. 창문은 어두운 거울이 되었고, 헤더의 반사는 두 겹, 세 겹으로 겹쳐졌다. 마치 세상이 그녀의 버전을 시험하는 것처럼.

그녀는 한 번 출간했던 단편을 떠올렸다. 그 작품을 읽은 사람은 인심 좋은 교수와 겨울빛에 대한 문장을 좋아하는 소규모 독자뿐이었다. 살짝 열었다가, 천천히 닫아버린 문 같았다. 아직 맡을 준비가 안 된 냄비의 김을 잠깐 빼듯이.

휴대폰 배터리는 8%였다. 그녀는 파일을 열지 않았다. 그걸 썼던 사람과 마주치고 싶지 않았다.

열차는 그녀의 역을 앞두고 거의 부모 같은 한숨을 쉬며 속도를 늦췄다. 플랫폼에 내리자, 차가움이 늘 그랬듯 코트를 뚫고 스며들었다. 계단을 오르자 밤과 그 온순한 비가 다시 시작되었다. 이 동네의 건물들은 스스로에 대한 확신이 덜했다. 창문들은 남의 삶을 위한 작은 극장이 되었다—어떤 창은 불이 켜져 있고, 어떤 창은 꺼져 있고, 어떤 창은 TV 화면의 푸른 바다빛으로 잠겨 있었다.

복도는 빨래 냄새와 정체 모를 달큰한 냄새가 섞여 났다. 집에 들어와 불 하나를 켰고, 야심 없는 빛이 자리를 잡도록 내버려 두었다. 창턱의 다육이는 지난주에 포기해버렸다. 그녀는 아직 치우지 않았다. 흙에는 물이 있던 작은 오목함만 남아 있었다.

헤더는 휴대폰을 꽂아 충전하고, 잠시 서서 집이 다시 조용해지는 소리를 들었다. 작은 탁자에 앉아 공책을 펼치고 마지막으로 쓴 페이지를 바라보았다. 흑연은 그녀의 손이 너무 세게 눌렀던 자리에서 은근히 반짝였다.
...
라디에이터가 한 번, 그리고 다시 한 번 똑딱거렸다. 메트로놈이 다시 맞춰지는 소리. 인도에서 누군가가 웃었고, 그 웃음은 얇은 창을 타고 작은, 반가운 실수처럼 올라왔다. 헤더는 공책을 덮고, 그 온기를 손바닥으로 감싸, 내일의 카페를 상상했다—벗겨진 페인트, 암기한 주문들로 줄을 선 단골들, 팁 단지 옆에서 식어가는 캐모마일.
